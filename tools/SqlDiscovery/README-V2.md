...
`ProjectSources` produce exclusivamente `connectionSource`, `databaseLookupSource`, `targetConnectionSource`, `metadataSource`, `physicalSource` e `historySource`. No crea declaraciones, repositorio, Registry, onboarding ni Schema Capture.
...
- La conexión target permanece separada y conserva exactamente `SUCCEEDED`, `AUTHENTICATION_FAILED`, `TRANSPORT_FAILED`, `TIMEOUT`, `CANCELLED` o `NOT_ATTEMPTED`.
